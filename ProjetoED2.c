//NOME:Julia Yukari Dairiki   RA:2422654
//NOME:Murilo Albuquerque De Araujo   RA:2510154
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
void capitalizar(char *nome); //Faz com que a primeira letra de cada palavra seja maiúscula
...
void capitalizar(char *nome) { //Faz com que a primeira letra de cada palavra seja maiúscula 
    ...
    int inicio_palavra = 1;  //Flag para identificar o início de uma palavra
	
    while(*nome){
        if(*nome == '_'){
            inicio_palavra = 1;    //Como temos um espaço, a próxima palavra precisa iniciar com letra maiúscula
        }else{
            if(inicio_palavra){
                *nome = toupper((unsigned char) *nome);  //Torna a primeira letra maiúscula
                inicio_palavra = 0;  //Desliga a flag até o próximo espaço
            }else{
                *nome = tolower((unsigned char) *nome);  //Torna as outras letras minúsculas
            }
        }
        nome++;
    }
