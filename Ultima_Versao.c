	...
	//Processo de leitura de um arquivo
		...
		while (fscanf(arq, "%d %s %d %s", &ID, nome, &idade, condicao_medica) == 4){
			...
			insereNome(&raiz_Nome, ID, nome, idade, condicao_medica); //Insere para organizar Nome;
			 ...
			 contador++; //Adiciona +1 toda vez que o laço se repete;
		}
			...
			while(op != 8){
				...
				switch(op){	
					...
					case 4:{
						...
						achouID = buscarID(raiz_ID, aux); //Utilização do achou por ID;
					...
					case 6: {
                		...
                		removerNome(&raiz_Nome, raiz_ID->nome); //Para remover o Nó na arvore Nome;
					...
					case 7: {
                		...
                		removerID(&raiz_ID, raiz_Nome->ID);//Para remover o Nó na arvore ID;
                		contador--;
            		}break;
					...
					//Caso o usuario coloque uma opção invalida
					default: 
...
//Algumas modificações de buscarPaiID;
